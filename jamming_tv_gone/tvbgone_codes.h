#ifndef TVBGONE_CODES_H
#define TVBGONE_CODES_H

#include <stdint.h>
#define NUM_ELEM(x) (sizeof (x) / sizeof (*(x)));
// Struktur untuk menyimpan kode IR
struct IrCode {
  uint8_t timer_val;
  uint8_t numpairs;
...
// Fungsi bantu
uint16_t freq_to_timerval(uint16_t freq) {
  return (freq > 0) ? (1000000UL / freq) : 1000;  // fallback ke 1000
}
...
freq_to_timerval(55556), // Gantilah dengan frekuensi IR yang benar, misalnya 38kHz ≈ 526
              // Non-pulsed code
  27,   // # of pairs
  2,    // # of bits per index
  code_na055Times,
